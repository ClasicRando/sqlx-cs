...
| UseExtendedProtocolForSimpleQueries | True if the driver should execute some simple statements as prepared queries. This does not impact simple statements that contain multiple queries or characters that look like parameter placeholders. This can improve performance by using binary encoding for results but also might slow down performance. Perform benchmarks before enabling feature. | 100                         |
