...
public partial class PgConnectionTest
{
    ...
    public async Task ExecuteScalar_Should_EncodeAndDecode_When_MacAddressAndDefaultEncoding(CancellationToken ct)
    {
        ...
        await using IPgExecutableQuery query = connection.CreateQuery("SELECT $1 macaddr_col;");
        query.Bind(value);
    ...
    public async Task ExecuteScalar_Should_EncodeAndDecode_When_MacAddress8AndDefaultEncoding(CancellationToken ct)
    {
        ...
        await using IPgConnection connection = DatabaseFixture.BasicPool.CreateConnection();
        await using IPgExecutableQuery query = connection.CreateQuery("SELECT $1 macaddr8_col;");
        query.Bind(value);
