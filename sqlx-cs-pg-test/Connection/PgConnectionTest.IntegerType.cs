...
public partial class PgConnectionTest
{
    ...
    public async Task ExecuteScalar_Should_EncodeAndDecode_When_ShortAndDefaultEncoding(CancellationToken ct)
    {
        ...
        await using IPgExecutableQuery query = connection.CreateQuery("SELECT $1 short_col;");
        query.Bind(value);
        var result = await query.ExecuteScalar<short>(ct);
    ...
    public async Task ExecuteScalar_Should_EncodeAndDecode_When_IntAndDefaultEncoding(CancellationToken ct)
    {
        ...
        await using IPgConnection connection = DatabaseFixture.BasicPool.CreateConnection();
        await using IPgExecutableQuery query = connection.CreateQuery("SELECT $1 int_col;");
        query.Bind(value);
    ...
    public async Task ExecuteScalar_Should_EncodeAndDecode_When_OidAndDefaultEncoding(CancellationToken ct)
    {
        ...
        await using IPgConnection connection = DatabaseFixture.BasicPool.CreateConnection();
        await using IPgExecutableQuery query = connection.CreateQuery("SELECT $1 oid_col;");
    ...
    public async Task ExecuteScalar_Should_EncodeAndDecode_When_LongAndDefaultEncoding(CancellationToken ct)
    {
        ...
        await using IPgConnection connection = DatabaseFixture.BasicPool.CreateConnection();
        await using IPgExecutableQuery query = connection.CreateQuery("SELECT $1 long_col;");
        query.Bind(value);
        var result = await query.ExecuteScalar<long>(ct);
        await Assert.That(result).IsEqualTo(value);
    }
